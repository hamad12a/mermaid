import { p as Lt, f as W } from "./flowDb-81d4a313.js";
import { h as S, u as Et, r as _t, p as Tt, l as At, d as D, f as tt, G as Nt } from "./layout-2c1637bb.js";
import { h as x, m as et, g as J, f as rt, l as M } from "./commonDb-ee617a3e.js";
import { a as A, b as at, i as nt, c as _, e as st, d as it, k as ot, f as Ct, g as It } from "./styles-0e47479c.js";
import { l as Mt } from "./line-a504cab0.js";
import { y as G, z as V, A as z, B as Bt } from "./utils-865f90f3.js";
import { s as Dt } from "./selectAll-3476431b.js";
import "./mermaidAPI-b39e5b34.js";
import "./index-fd07e60c.js";
...
function K(r, e, t) {
  ...
  e.labelType === "svg" ? Yt(a, e) : typeof n != "string" || e.labelType === "html" ? at(a, e) : Ht(a, e);
...
var q = function(r, e, t) {
  ...
  a.exit().remove(), a.enter().append("g").attr("class", "node").style("opacity", 0), a = r.selectAll("g.node"), a.each(function(i) {
    ...
    var d = c.append("g").attr("class", "label"), l = K(d, o), y = t[o.shape], u = Tt(l.node().getBBox(), "width", "height");
    o.elem = this, o.id && c.attr("id", o.id), o.labelId && d.attr("id", o.labelId), S(o, "width") && (u.width = o.width), S(o, "height") && (u.height = o.height), u.width += o.paddingLeft + o.paddingRight, u.height += o.paddingTop + o.paddingBottom, d.attr(
      "transform",
      "translate(" + (o.paddingLeft - o.paddingRight) / 2 + "," + (o.paddingTop - o.paddingBottom) / 2 + ")"
    );
    var h = x(this);
    h.select(".label-container").remove();
    var p = y(h, u, o).classed("label-container", !0);
    A(p, o.style);
...
function se(r, e, t, n) {
  var a, s, i, o, c, d, l, y, u, h, p, m, f, v, k;
  if (a = e.y - r.y, i = r.x - e.x, c = e.x * r.y - r.x * e.y, u = a * t.x + i * t.y + c, h = a * n.x + i * n.y + c, !(u !== 0 && h !== 0 && j(u, h)) && (s = n.y - t.y, o = t.x - n.x, d = n.x * t.y - t.x * n.y, l = s * r.x + o * r.y + d, y = s * e.x + o * e.y + d, !(l !== 0 && y !== 0 && j(l, y)) && (p = a * o - s * i, p !== 0)))
...
function T(r, e, t) {
  ...
  for (var c = n - r.width / 2 - i, d = a - r.height / 2 - o, l = 0; l < e.length; l++) {
    var y = e[l], u = e[l < e.length - 1 ? l + 1 : 0], h = se(
      r,
      t,
      { x: c + y.x, y: d + y.y },
      { x: c + u.x, y: d + u.y }
  ...
  return s.length ? (s.length > 1 && s.sort(function(p, m) {
    var f = p.x - t.x, v = p.y - t.y, k = Math.sqrt(f * f + v * v), C = m.x - t.x, E = m.y - t.y, $ = Math.sqrt(C * C + E * E);
    return k < $ ? -1 : k === $ ? 0 : 1;
...
function he() {
  ...
  return r.createNodes = function(e) {
    ...
    return arguments.length ? (Ft(e), r) : H;
  }, r.createEdgeLabels = function(e) {
    return arguments.length ? (qt(e), r) : X;
  }, r.createEdgePaths = function(e) {
    return arguments.length ? (Qt(e), r) : F;
  }, r.shapes = function(e) {
    return arguments.length ? (ie(e), r) : Q;
  }, r.arrows = function(e) {
...
function pe(r) {
  r.nodes().forEach(function(e) {
    ...
    }), D(t, ue), tt(["paddingLeft", "paddingRight", "paddingTop", "paddingBottom"], function(n) {
      t[n] = Number(t[n]);
    }), S(t, "width") && (t._prevWidth = t.width), S(t, "height") && (t._prevHeight = t.height);
  }), r.edges().forEach(function(e) {
    var t = r.edge(e);
    S(t, "label") || (t.label = ""), D(t, fe);
  });
}
function ye(r) {
...
function xt(r, e, t) {
  ...
  return t.intersect = function(d) {
    ...
    if (a != 0 && (Math.abs(y) < t.width / 2 || Math.abs(y) == t.width / 2 && Math.abs(l.y - t.y) > t.height / 2 - s)) {
      let u = s * s * (1 - y * y / (a * a));
      u != 0 && (u = Math.sqrt(u)), u = s - u, d.y - t.y > 0 && (u = -u), l.y += u;
    }
    return l;
  }, c;
}
function ve(r) {
  r.shapes().question = dt, r.shapes().hexagon = ht, r.shapes().stadium = mt, r.shapes().subroutine = wt, r.shapes().cylinder = xt, r.shapes().rect_left_inv_arrow = ut, r.shapes().lean_right = ft, r.shapes().lean_left = pt, r.shapes().trapezoid = yt, r.shapes().inv_trapezoid = vt, r.shapes().rect_right_inv_arrow = gt;
}
function ge(r) {
  r({ question: dt }), r({ hexagon: ht }), r({ stadium: mt }), r({ subroutine: wt }), r({ cylinder: xt }), r({ rect_left_inv_arrow: ut }), r({ lean_right: ft }), r({ lean_left: pt }), r({ trapezoid: yt }), r({ inv_trapezoid: vt }), r({ rect_right_inv_arrow: gt });
}
function N(r, e, t, n) {
...
const me = {
  ...
  Object.keys(r).forEach(function(d) {
    ...
    if (et(J().flowchart.htmlLabels)) {
      const v = {
        label: h.replace(/fa[blrs]?:fa-[\w-]+/g, (k) => `<i class='${k.replace(":", " ")}'></i>`).replace(
          /\$\$(.*)\$\$/g,
          (k, C) => ot.renderToString(C, { throwOnError: !0, displayMode: !0, output: "mathml" }).replace(/\n/g, " ").replace(/<annotation.*<\/annotation>/g, "")
        )
      };
      ...
      const k = h.split(rt.lineBreakRegex);
      for (const C of k) {
        const E = o.createElementNS("http://www.w3.org/2000/svg", "tspan");
        E.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve"), E.setAttribute("dy", "1em"), E.setAttribute("x", "1"), E.textContent = C, v.appendChild(E);
      }
      p = v;
    }
    let m = 0, f = "";
  ...
  r.forEach(function(i) {
    ...
    l.style = y, l.labelStyle = u, i.interpolate !== void 0 ? l.curve = z(i.interpolate, G) : r.defaultInterpolate !== void 0 ? l.curve = z(r.defaultInterpolate, G) : l.curve = z(bt.curve, G), i.text === void 0 ? i.style !== void 0 && (l.arrowheadStyle = "fill: #333") : (l.arrowheadStyle = "fill: #333", l.labelpos = "c", et(J().flowchart.htmlLabels) ? (l.labelType = "html", l.label = `<span id="L-${o}" class="edgeLabel L-${c}' L-${d}" style="${l.labelStyle}">${i.text.replace(/fa[blrs]?:fa-[\w-]+/g, (h) => `<i class='${h.replace(":", " ")}'></i>`).replace(
      /\$\$(.*)\$\$/g,
      (h, p) => ot.renderToString(p, { throwOnError: !0, displayMode: !0, output: "mathml" }).replace(/\n/g, " ").replace(/<annotation.*<\/annotation>/g, "")
...
`), i.style === void 0 && (l.style = l.style || "stroke: #333; stroke-width: 1.5px;fill:none"), l.labelStyle = l.labelStyle.replace("color:", "fill:"))), l.id = o, l.class = c + " " + d, l.minlen = i.length || 1, e.setEdge(t.db.lookUpDomId(i.start), t.db.lookUpDomId(i.end), l, n);
  ...
  try {
    ...
    M.debug("Parsing failed");
  }
  let d = n.db.getDirection();
  d === void 0 && (d = "TD");
  const l = s.nodeSpacing || 50, y = s.rankSpacing || 50, u = new Nt({
    multigraph: !0,
    compound: !0
  }).setGraph({
    rankdir: d,
  ...
  for (let g = p.length - 1; g >= 0; g--)
    h = p[g], n.db.addVertex(h.id, h.title, "group", void 0, h.classes);
  const m = n.db.getVertices();
  M.warn("Get vertices", m);
  const f = n.db.getEdges();
  let v = 0;
  for (v = p.length - 1; v >= 0; v--) {
    h = p[v], Dt("cluster").append("text");
    for (let g = 0; g < h.nodes.length; g++)
      M.warn(
        "Setting subgraph",
        h.nodes[g],
        n.db.lookUpDomId(h.nodes[g]),
        n.db.lookUpDomId(h.id)
      ), u.setParent(n.db.lookUpDomId(h.nodes[g]), n.db.lookUpDomId(h.id));
  }
  kt(m, u, e, o, c, n), St(f, u, n);
  const k = new he();
  me.addToRender(k), k.arrows().none = function(b, L, w, B) {
    const I = b.append("marker").attr("id", L).attr("viewBox", "0 0 10 10").attr("refX", 9).attr("refY", 5).attr("markerUnits", "strokeWidth").attr("markerWidth", 8).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 0 0 L 0 0 L 0 0 z");
    A(I, w[B + "Style"]);
  ...
  const C = o.select(`[id="${e}"]`), E = o.select("#" + e + " g");
  for (k(E, u), E.selectAll("g.node").attr("title", function() {
    ...
    if (h = p[v], h.title !== "undefined") {
      ...
      ), L = g[0].x.baseVal.value, w = g[0].y.baseVal.value, B = g[0].width.baseVal.value, I = x(b[0]).select(".label");
      I.attr("transform", `translate(${L + B / 2}, ${w + 14})`), I.attr("id", e + "Text");
      for (let U = 0; U < h.classes.length; U++)
        b[0].classList.add(h.classes[U]);
    }
  if (!s.htmlLabels) {
  ...
  Bt(u, C, s.diagramPadding, s.useMaxWidth), Object.keys(m).forEach(function(g) {
    const b = m[g];
    if (b.link) {
  ...
  styles: It,
  init: (r) => {
    r.flowchart || (r.flowchart = {}), r.flowchart.arrowMarkerAbsolute = r.arrowMarkerAbsolute, ke.setConf(r.flowchart), W.clear(), W.setGen("gen-1");
  }
};
export {
  Ge as diagram
};
//# sourceMappingURL=flowDiagram-edb8a0be.js.map
